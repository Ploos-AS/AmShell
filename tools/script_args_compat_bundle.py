#!/usr/bin/env python3
"""Build the M1.7 script-argument runtime qualification package."""
...
def main() -> int:
    ...
    shutil.copy2(ROOT / "tests" / "compat" / "scripts" / "args.script", OUT / "args.script")
    shutil.copy2(CASES, OUT / "script_args_cases.tsv")

    guest = [
        "; AmShell M1.7 script-argument differential qualification",
        "FailAt 20",
        "MakeDir T:AmShellM17 >NIL:",
    ]

    for case_id, arguments in load_cases():
        native_command = f"Execute args.script {arguments}"
        ...
        guest.extend(
            [
                ...
                f"AmShell args.script {arguments} >T:AmShellM17/amshell-{case_id}.out",
                f"Echo $RC >T:AmShellM17/amshell-{case_id}.rc",
            ]
        )

    guest.append("Echo M1.7 guest run complete")
    (OUT / "run-qualification.script").write_text(
        "\n".join(guest) + "\n", encoding="ascii", newline="\n"
    )

    (OUT / "README.txt").write_text(
        "AmShell M1.7 script-argument runtime qualification\n\n"
        ...
        "outer argument text. No output normalization is expected for this fixture.\n",
        encoding="ascii",
        newline="\n",
    )

    print(f"Prepared M1.7 qualification bundle in {OUT}")
    return 0
